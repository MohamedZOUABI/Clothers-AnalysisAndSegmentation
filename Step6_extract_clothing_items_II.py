...
class ClothingExtractor:
    ...
    def process_image(self, image_path: str, output_dir: str = 'extracted_items'):
        ...
        for i in range(len(predictions['boxes'])):
            ...
            cropped_item_rgba[:, :, 3] = cropped_mask * 255  # Alpha channel from mask
            
            # Save only the PNG with transparent background
            item_filename_png = f"{base_name}_item{i+1}_{category}.png"
...
def main():
    ...
    for item in results['items']:
        ...
        print(f"  File: {os.path.basename(item['image_file'])}")
        print()
